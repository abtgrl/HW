...
def process():
    if not open_dir:
        ...
        image_paths = [os.path.join(open_dir, filename) for filename in os.listdir(open_dir)
        ...
        for image in image_paths:
            thread = threading.Thread(target=edit_images, args=(image, status, filters, save_dir))
...
file_frame = tk.Frame(root, pady=10, padx=10)
...
filters_frame = tk.Frame(root, padx=30)
...
tk.Button(root, text='Обработать', command=process).grid(row=2, column=0, columnspan=2)
status = tk.Label(root, text='')
status.grid(row=3, column=0, columnspan=2)
root.rowconfigure(3, minsize=45)
